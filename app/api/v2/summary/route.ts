// v2 (Next.js Route Handler) for the production cron at /api/v2/summary.
// Full handler lives here (migrated from src/api/summary.ts); it imports the
// same stats/recap/announce modules the legacy bundled `api/summary.mjs` (v1)
// uses, so behaviour is identical while both paths exist. The weekly recap
...
import { announceWeeklyRecap } from "@/src/utils/announce";
import { linkMentions } from "@/src/utils/format";
import {
  collectWeeklyStats,
  generateWeeklyRecap,
  hadActivity,
