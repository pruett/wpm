...
import { groupBySeries, isNotModified, kickoffLabel, menuHandle, seriesHeaderRow } from "./placebet";
...
/**
 * Events the /showbets list shows — broader than /placebet's menu. Everything still
 * bettable (same pre-game window /placebet offers), plus any event with open
 * bets riding on it: betting locks at kickoff, but a running game's bets
 * are exactly what /showbets exists to show. In-play events with no open bets
...
async function listedEvents(openBetEventTickers: string[]) {
  ...
  // Same pre-game window /placebet offers: kickoff in the future (betting locks
...
// Event list grouped by series like /placebet's menu, but each button shows how
      ...
      ? [
          {
            ...
            content: live
              ...
              : "No open bets yet — type /placebet to place the first one.",
          },
        ]
      : rows.map((bet, i) => ({
          type: "text" as const,
...
// The /showbets card is a shared read-only view (unlike /placebet's per-user menus),
...
export const showbets: BotCommand = {
  name: "showbets",
