...
- [ ] Reshape `markets` table in the schema: drop `sport`/`teamA`/`teamB`/`tickerA`/`tickerB`/`closesAt`/`resolvedOutcome`; add `eventId`/`name`/`ticker`/`resolvedAs`. Move `sport` to `events`.
...
- [ ] Void semantics: all children settled `no` → all `cancelled_voided`.
...
- [ ] `commit` with all `cancelled_voided` when every child settled `no`.
